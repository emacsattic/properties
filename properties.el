...
(defcustom properties-categories
  `(("Mode"
     (:setter funcall)
     (:reader (lambda ()
  ...
  "*A list of categories know to `properties-add-special'.
...
(defvar properties-etc nil
  "An plist of other things to add.
Categories are defined by `properties-categories'.")
(make-variable-buffer-local 'properties-etc)
...
(defun properties-save-properties-file-p (&optional properties overlays etc)
  ...
  (when (and (not properties-tmp-buffer)
	...
	(let ((ols      (properties-get-overlay-list))
	  ...
	  (with-temp-buffer
	    ...
	    (when (properties-save-properties-file-p proplst ols etc)
  ...
  (let* ((file     (or file
	...
	(dolist (this vars)
	  (let ((var (car this))
	    ...
	    (set var val)))
	;;restore etc
	(setq properties-etc etc)
	(dolist (this etc)
	  (let* ((cat (car this))
		 (setter (tellib-alist-get (assoc cat properties-categories)
					   :setter nil t)))
...
;;;###autoload
(defun properties-add-special (&optional category value)
  "Add something being defined in `properties-categories' to `properties-etc'."
  (interactive)
  (let* ((cat (or category
		  (completing-read "Category: " properties-categories nil t)))
	 (val (or value
		  (funcall (tellib-alist-get (assoc cat properties-categories)
					     :reader nil t)))))
    (setq properties-etc
	  (tellib-alist-set properties-etc
			    cat
			    (cons val
				  (tellib-alist-get properties-etc cat))))))

;;;###autoload
(defun properties-remove-special (&optional category value)
  "Remove something being defined in `properties-categories' to `properties-etc'."
  (interactive)
  (let* ((cat (or category
		  (completing-read "Category: " properties-categories nil t)))
	 (val (or value
    ...
    (setq properties-etc
	  (tellib-alist-set properties-etc
			    cat
			    (remove val
				    (tellib-alist-get properties-etc
						      cat))))))
